        ...
        register_instance_option :controller do
          Proc.new do
            ...
            Prawn::Document.new(PDF_OPTIONS) do |pdf|
              ...
              pdf.text "RelatÃ³rio do Representante", :size => 32, :style => :bold, :align => :center
              pdf.move_down 80
              ...
              if @object.comissions.where(status: :pending).count > 0
                ...
                pdf.text "ComissÃµes Pendentes", :size => 14, :align => :justify, :inline_format => true, :style => :bold
                pdf.move_down 14
              ...
              if @object.sales.count > 0
                ...
                # Gera a imagem no diretÃ³rio pÃºblico (vocÃª pode escolher onde gerar)
                g.write("public/#{ramdom_file_name}_graph.jpg")

                pdf.start_new_page

                pdf.text "GrÃ¡fico de Vendas", :size => 20, :style => :bold, :align => :center

                # Incluir o grÃ¡fico numero 2
              ...
              pdf.font "Helvetica"
              # Inclui um texto com um link clicÃ¡vel (usando a tag link) no bottom da folha do lado esquerdo e coloca uma cor especifica nessa parte (usando a tag color)
              pdf.text "Link Para o Manul do Prawn<link href='http://prawnpdf.org/manual.pdf'> <color rgb='5ca3e6'>clicÃ¡vel</color></link>", :size => 10, :inline_format => true, :valign => :bottom, :align => :left
              # Inclui em baixo da folha do lado direito a data e o nÃ©mero da pÃ¡gina usando a tag page
              pdf.number_pages "Gerado: #{(Time.now).strftime("%d/%m/%y as %H:%M")} - PÃ¡gina <page>", :start_count_at => 0, :page_filter => :all, :at => [pdf.bounds.right - 140, 7], :align => :right, :size => 8
